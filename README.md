...
- **中英文分开**：批注记录创建时的界面语言，只在同一语言下显示（中英正文是两份独立内容，没法逐句对应）。没有 `locale` 字段的条目两种语言都显示，比如划在两边共用的代码块上的旧数据。存下的翻译不分语言，在另一种语言界面划同一段文字照样直接复用
- 早期版本存在浏览器 `localStorage` 里的批注，打开 `npm run dev` 时会自动搬进文件（这类数据没有语言信息，两种语言都会显示）
